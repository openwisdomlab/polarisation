#!/usr/bin/env python3
"""
Stokes Vector Demonstration - OPTIMIZED
斯托克斯矢量演示 - 优化版
...
OPTIMIZATIONS: Unified PolarCraft dark theme, enhanced controls, smooth animations
...
class StokesVectorDemo:
    ...
    def __init__(self):
        ...
        self.fig.suptitle('Stokes Vector Demonstration / 斯托克斯矢量演示 - OPTIMIZED',
                          fontsize=FONTS['title']['size'], color=COLORS['text_primary'], y=0.98)
    ...
    def setup_figure(self):
        ...
        self.ax_ellipse.set_facecolor(COLORS['surface'])
    ...
    def setup_controls(self):
        ...
        # Slider: S₁ (H-V)
        ax_s1 = plt.axes([0.15, 0.14, 0.25, 0.02], facecolor=COLORS['surface'])
        self.slider_s1 = Slider(ax_s1, 'S₁ (H-V)', -1, 1, valinit=1, valstep=0.01,
                                color=COLORS['primary'])
        self.slider_s1.on_changed(self.update)
        style_slider(self.slider_s1, COLORS['primary'])

        # Slider: S₂ (±45°)
        ax_s2 = plt.axes([0.15, 0.10, 0.25, 0.02], facecolor=COLORS['surface'])
        self.slider_s2 = Slider(ax_s2, 'S₂ (±45°)', -1, 1, valinit=0, valstep=0.01,
                                color=COLORS['secondary'])
        self.slider_s2.on_changed(self.update)
        style_slider(self.slider_s2, COLORS['secondary'])

        # Slider: S₃ (R-L)
        ax_s3 = plt.axes([0.15, 0.06, 0.25, 0.02], facecolor=COLORS['surface'])
        self.slider_s3 = Slider(ax_s3, 'S₃ (R-L)', -1, 1, valinit=0, valstep=0.01,
                                color=COLORS['warning'])
        self.slider_s3.on_changed(self.update)
        style_slider(self.slider_s3, COLORS['warning'])

        # Slider: DOP
        ax_dop = plt.axes([0.15, 0.02, 0.25, 0.02], facecolor=COLORS['surface'])
        self.slider_dop = Slider(ax_dop, 'DOP', 0, 1, valinit=1, valstep=0.01,
                                 color=COLORS['success'])
        self.slider_dop.on_changed(self.update)
        style_slider(self.slider_dop, COLORS['success'])

        # Radio buttons: Preset states
        ax_radio = plt.axes([0.52, 0.02, 0.15, 0.14], facecolor=COLORS['surface'])
        ...
        self.btn_reset = Button(ax_reset, 'Reset', color=COLORS['surface'],
    ...
    def plot_stokes_bars(self):
        ...
        # Bar positions
        labels = ['S₀', 'S₁', 'S₂', 'S₃']
        colors = [COLORS['primary'], COLORS['polarized_h'], COLORS['warning'], COLORS['secondary']]

        # Plot bars
        bars = self.ax_bars.bar(labels, S, color=colors, alpha=0.8,
        ...
        for bar, val in zip(bars, S):
            ...
            self.ax_bars.text(bar.get_x() + bar.get_width()/2, height,
                             ...
                             color=COLORS['text_primary'], fontsize=FONTS['label']['size'])

        # Styling
        self.ax_bars.set_ylabel('Value', color=COLORS['text_primary'],
                                fontsize=FONTS['label']['size'])
        self.ax_bars.set_ylim(-1.2, 1.2)
        self.ax_bars.axhline(0, color=COLORS['text_primary'], linewidth=0.5, alpha=0.3)
        self.ax_bars.grid(True, alpha=0.3, color=COLORS['grid'])
        self.ax_bars.tick_params(colors=COLORS['tick'], labelsize=FONTS['tick']['size'])
        self.ax_bars.set_title('Stokes Parameters / 斯托克斯参数',
                              color=COLORS['text_primary'], fontsize=FONTS['subtitle']['size'])
    ...
    def plot_poincare_sphere(self):
        ...
        self.ax_poincare.plot_surface(x_sphere, y_sphere, z_sphere,
                                      color=COLORS['primary'], alpha=0.1, linewidth=0)

        # Draw axes
        axis_length = 1.3
        self.ax_poincare.plot([-axis_length, axis_length], [0, 0], [0, 0],
                             color=COLORS['polarized_h'], linewidth=2, alpha=0.6, label='S₁ (H-V)')
        self.ax_poincare.plot([0, 0], [-axis_length, axis_length], [0, 0],
                             color=COLORS['success'], linewidth=2, alpha=0.6, label='S₂ (±45°)')
        self.ax_poincare.plot([0, 0], [0, 0], [-axis_length, axis_length],
                             color=COLORS['secondary'], linewidth=2, alpha=0.6, label='S₃ (R-L)')

        # Labels
        self.ax_poincare.text(axis_length, 0, 0, 'H', color=COLORS['polarized_h'],
                             fontsize=FONTS['label']['size'])
        self.ax_poincare.text(-axis_length, 0, 0, 'V', color=COLORS['polarized_h'],
                             fontsize=FONTS['label']['size'])
        self.ax_poincare.text(0, axis_length, 0, '+45°', color=COLORS['success'],
                             fontsize=FONTS['label']['size'])
        self.ax_poincare.text(0, -axis_length, 0, '-45°', color=COLORS['success'],
                             fontsize=FONTS['label']['size'])
        self.ax_poincare.text(0, 0, axis_length, 'L', color=COLORS['secondary'],
                             fontsize=FONTS['label']['size'])
        self.ax_poincare.text(0, 0, -axis_length, 'R', color=COLORS['secondary'],
                             fontsize=FONTS['label']['size'])

        # Plot current state
        s1, s2, s3 = self.stokes.to_poincare()
        self.ax_poincare.scatter([s1], [s2], [s3], c=COLORS['danger'], s=200,
                                marker='o', edgecolors=COLORS['text_primary'], linewidths=2,
                                label='Current State')

        # Draw line from origin to state (showing DOP)
        self.ax_poincare.plot([0, s1], [0, s2], [0, s3], color=COLORS['text_primary'],
        ...
        self.ax_poincare.set_xlabel('S₁', color=COLORS['text_primary'],
                                   fontsize=FONTS['label']['size'])
        self.ax_poincare.set_ylabel('S₂', color=COLORS['text_primary'],
                                   fontsize=FONTS['label']['size'])
        self.ax_poincare.set_zlabel('S₃', color=COLORS['text_primary'],
                                   fontsize=FONTS['label']['size'])
        self.ax_poincare.tick_params(colors=COLORS['tick'], labelsize=FONTS['tick']['size'])

        # Dark background
        self.ax_poincare.xaxis.pane.fill = False
        self.ax_poincare.yaxis.pane.fill = False
        self.ax_poincare.zaxis.pane.fill = False
        self.ax_poincare.xaxis.pane.set_edgecolor(COLORS['axis'])
        self.ax_poincare.yaxis.pane.set_edgecolor(COLORS['axis'])
        self.ax_poincare.zaxis.pane.set_edgecolor(COLORS['axis'])
    ...
    def plot_ellipse(self):
        ...
        self.ax_ellipse.set_aspect('equal')
        self.ax_ellipse.grid(True, alpha=0.3, color=COLORS['grid'])

        dop = self.stokes.dop()

        if dop < 0.01:
            # Unpolarized - draw circle
            circle = Circle((0, 0), 0.3, edgecolor=COLORS['text_muted'], facecolor='none',
            ...
            self.ax_ellipse.plot(x_line, y_line, 'o-', color=COLORS['primary'],
                                linewidth=SIZES['line_thick'], markersize=SIZES['marker_large'])
        else:
        ...
        # Labels
        self.ax_ellipse.set_xlabel('Ex', color=COLORS['text_primary'],
                                  fontsize=FONTS['label']['size'])
        self.ax_ellipse.set_ylabel('Ey', color=COLORS['text_primary'],
                                  fontsize=FONTS['label']['size'])
        self.ax_ellipse.tick_params(colors=COLORS['tick'], labelsize=FONTS['tick']['size'])

        # Info text
        info_text = f'ψ = {psi_deg:.1f}°\nχ = {chi_deg:.1f}°\n{handedness.upper()}'
        self.ax_ellipse.text(0.05, 0.95, info_text, transform=self.ax_ellipse.transAxes,
                            fontsize=FONTS['label']['size'], color=COLORS['text_primary'], va='top',
    ...
    def plot_intensities(self):
        ...
        colors = [COLORS['primary'], COLORS['info'], COLORS['warning'],
                 COLORS['polarized_45'], COLORS['secondary'], COLORS['polarized_135']]
        ...
        # Styling
        self.ax_intensity.set_ylabel('Intensity', color=COLORS['text_primary'],
                                    fontsize=FONTS['label']['size'])
        self.ax_intensity.set_ylim(0, 1.2)
        self.ax_intensity.grid(True, alpha=0.3, color=COLORS['grid'], axis='y')
        self.ax_intensity.tick_params(colors=COLORS['tick'], labelsize=FONTS['tick']['size'])
        self.ax_intensity.set_title('Intensity Measurements / 强度测量',
                                   color=COLORS['text_primary'], fontsize=FONTS['subtitle']['size'])
    ...
    def plot_dop_gauge(self):
        ...
        x_arc = np.cos(theta)
        y_arc = np.sin(theta)
        self.ax_dop.plot(x_arc, y_arc, color=COLORS['text_primary'], linewidth=3, alpha=0.3)
        ...
        self.ax_dop.text(0, 0.5, f'{dop:.3f}', ha='center', va='center',
                        color=COLORS['text_primary'], fontsize=32, weight='bold')

        self.ax_dop.text(0, -0.2, f'DOP = {dop*100:.1f}%', ha='center',
                        color=COLORS['text_primary'], fontsize=FONTS['subtitle']['size'])

        # Labels
        self.ax_dop.text(-1.1, 0, '0', color=COLORS['text_primary'],
                        fontsize=FONTS['label']['size'], ha='right')
        self.ax_dop.text(1.1, 0, '1', color=COLORS['text_primary'],
                        fontsize=FONTS['label']['size'], ha='left')
        self.ax_dop.text(0, 1.1, 'Fully\nPolarized', color=COLORS['text_primary'],
                        fontsize=FONTS['label']['size'], ha='center')
...
DECOMPOSITION (分解):
  Polarized: S_pol = [{S_pol.S[0]:.4f}, {S_pol.S[1]:.4f}, {S_pol.S[2]:.4f}, {S_pol.S[3]:.4f}]ᵀ
  Unpolarized: S_unpol = [{S_unpol.S[0]:.4f}, {S_unpol.S[1]:.4f}, {S_unpol.S[2]:.4f}, {S_unpol.S[3]:.4f}]ᵀ
        """

        self.ax_params.text(0.05, 0.95, info_text, transform=self.ax_params.transAxes,
                           fontsize=FONTS['label']['size'], color=COLORS['text_primary'],
                           va='top', family='monospace',
                           bbox=dict(boxstyle='round', facecolor=COLORS['surface'], alpha=0.9,
...
def main():
    ...
    print("="*70)
    print("Stokes Vector Demonstration - OPTIMIZED / 斯托克斯矢量演示 - 优化版")
    print("="*70)
    ...
    print("\nOptimizations (优化):")
    print("  ✓ Unified PolarCraft dark theme")
