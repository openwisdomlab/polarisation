#!/usr/bin/env python3
"""
Jones矩阵演示 - 优化版 (Jones Matrix Demonstration - OPTIMIZED)
==============================================================

完整的Jones矩阵偏振光学演示程序，包含Jones矢量、Jones矩阵计算、
偏振态变换可视化以及Poincare球3D显示。

OPTIMIZATIONS: 统一PolarCraft深色主题，增强交互控件，流畅动画
...
def draw_poincare_sphere(ax):
    ...
    # 绘制半透明球面
    ax.plot_surface(x, y, z, alpha=0.1, color=COLORS['primary'], linewidth=0)
    ...
    for phi in [0, np.pi/4, np.pi/2, 3*np.pi/4]:
        ax.plot(np.cos(phi)*np.sin(theta), np.sin(phi)*np.sin(theta), np.cos(theta),
                color=COLORS['text_muted'], linewidth=0.5, linestyle=':')

    # 坐标轴
    ax.plot([-1.3, 1.3], [0, 0], [0, 0], color=COLORS['polarized_h'], linewidth=2)
    ax.plot([0, 0], [-1.3, 1.3], [0, 0], color=COLORS['success'], linewidth=2)
    ax.plot([0, 0], [0, 0], [-1.3, 1.3], color=COLORS['primary'], linewidth=2)

    # 轴标签
    ax.text(1.5, 0, 0, 'S₁ (H-V)', color=COLORS['polarized_h'], fontsize=FONTS['annotation']['size'])
    ax.text(0, 1.5, 0, 'S₂ (±45°)', color=COLORS['success'], fontsize=FONTS['annotation']['size'])
    ax.text(0, 0, 1.5, 'S₃ (R-L)', color=COLORS['primary'], fontsize=FONTS['annotation']['size'])
    ...
    for s1, s2, s3, label in special_points:
        ax.scatter([s1], [s2], [s3], color=COLORS['warning'], s=30, marker='o')
        ax.text(s1*1.15, s2*1.15, s3*1.15, label, color=COLORS['warning'],
                fontsize=FONTS['annotation']['size'], ha='center', va='center')
...
def draw_jones_matrix_heatmap(ax, matrix, title='Jones Matrix'):
    ...
    for i in range(2):
        for j in range(2):
            # 实部
            val_r = real_part[i, j]
            color = COLORS['text_primary'] if abs(val_r) > vmax * 0.5 else COLORS['text_secondary']
            ax.text(j, i, f'{val_r:.2f}', ha='center', va='center',
                   color=color, fontsize=FONTS['label']['size'], fontweight='bold')
            # 虚部
            val_i = imag_part[i, j]
            color = COLORS['text_primary'] if abs(val_i) > vmax * 0.5 else COLORS['text_secondary']
            suffix = 'i' if val_i != 0 else ''
            ax.text(j + 2, i, f'{val_i:.2f}{suffix}', ha='center', va='center',
                   color=color, fontsize=FONTS['label']['size'], fontweight='bold')

    # 分隔线
    ax.axvline(x=1.5, color=COLORS['text_primary'], linewidth=2)

    # 标签
    ax.set_xticks([0.5, 2.5])
    ax.set_xticklabels(['Real', 'Imag'], color=COLORS['text_primary'])
    ax.set_yticks([0, 1])
    ax.set_yticklabels(['Ex→', 'Ey→'], color=COLORS['text_primary'])
    ax.tick_params(colors=COLORS['text_primary'], length=0)

    ax.set_title(title, color=COLORS['text_primary'],
                fontsize=FONTS['subtitle']['size'], fontweight='bold')
...
def draw_polarization_ellipse(ax, jones_vector, color=COLORS['primary']):
    """
    绘制偏振椭圆
    ...
    # 参考圆
    theta = np.linspace(0, 2*np.pi, 100)
    ax.plot(np.cos(theta), np.sin(theta), ':', color=COLORS['text_muted'],
            linewidth=0.5, alpha=0.5)

    # 坐标轴
    ax.axhline(0, color=COLORS['axis'], linewidth=0.5, linestyle=':')
    ax.axvline(0, color=COLORS['axis'], linewidth=0.5, linestyle=':')
    ...
    if pol_type == 'linear':
        ...
        arrow = FancyArrowPatch((-dx, -dy), (dx, dy),
                               ...
                               linewidth=SIZES['line_thick'], color=color)
        ax.add_patch(arrow)

        ax.text(0, -1.3, f"Linear\n{params['angle']:.1f}°",
               ha='center', va='top', color=color, fontsize=FONTS['annotation']['size'])

    elif pol_type in ['circular-r', 'circular-l']:
        # 圆偏振 - 圆圈 + 旋转方向
        circle = Circle((0, 0), 1, fill=False, edgecolor=color,
        ...
        ax.text(0, -1.3, label, ha='center', va='top', color=color,
        ...
        ax.plot(xr, yr, color=color, linewidth=SIZES['line_thick'])
...
class JonesMatrixDemo:
    ...
    def __init__(self):
        ...
        self.fig.suptitle('Jones矩阵演示 (Jones Matrix Demonstration)',
                         fontsize=FONTS['title']['size'], fontweight='bold',
    ...
    def create_layout(self):
        ...
        self.ax_matrix.set_facecolor(COLORS['surface'])
    ...
    def create_controls(self):
        ...
        ax_input_type.set_facecolor(COLORS['surface'])
        ...
        for label in self.radio_input.labels:
            ...
            label.set_fontsize(FONTS['annotation']['size'])
        ...
        self.slider_input_angle = Slider(
            ...
            valstep=5, color=COLORS['warning']
        )
        self.slider_input_angle.on_changed(self.on_input_angle_changed)
        style_slider(self.slider_input_angle, COLORS['warning'])

        # 光学元件类型（单选按钮）
        ax_element_type = plt.axes([0.02, 0.45, 0.10, 0.25])
        ax_element_type.set_facecolor(COLORS['surface'])
        ...
        self.slider_element_angle = Slider(
            ...
            valstep=5, color=COLORS['primary']
        )
        self.slider_element_angle.on_changed(self.on_element_angle_changed)
        style_slider(self.slider_element_angle, COLORS['primary'])

        # 重置按钮
        ax_reset = plt.axes([0.85, 0.08, 0.08, 0.04])
        self.btn_reset = Button(ax_reset, 'Reset',
                               color=COLORS['surface'], hovercolor=COLORS['surface_light'])
    ...
    def update_plot(self):
        ...
        # 3. 绘制输入偏振态
        self.ax_input.set_title('Input', color=COLORS['text_primary'],
                               fontsize=FONTS['subtitle']['size'])
        draw_polarization_ellipse(self.ax_input, input_jones, COLORS['warning'])

        # 4. 绘制输出偏振态
        self.ax_output.set_title('Output', color=COLORS['text_primary'],
                                fontsize=FONTS['subtitle']['size'])
        output_color = COLORS['success'] if output_jones.intensity() > 0.01 else COLORS['text_muted']
    ...
    def draw_optical_path(self, input_jones, jones_matrix, output_jones, element_name):
        ...
        # 光源
        source = Circle((1, 2.5), 0.4, color=COLORS['warning'], alpha=0.8)
        ax.add_patch(source)
        ax.text(1, 1.7, 'Light Source', ha='center', va='top',
               color=COLORS['text_secondary'], fontsize=FONTS['annotation']['size'])
        ...
        ax.arrow(1.5, 2.5, 1.8, 0, head_width=0.15, head_length=0.2,
                ...
                alpha=min(input_intensity, 1), linewidth=SIZES['line_thick'])
        ...
        if input_type == 'linear':
            ...
            arrow = FancyArrowPatch((2.5 - dx, 2.5 - dy), (2.5 + dx, 2.5 + dy),
                                   ...
                                   linewidth=SIZES['line_thick'], color=COLORS['warning'])
            ax.add_patch(arrow)
        elif input_type in ['circular-r', 'circular-l']:
            circ = Circle((2.5, 2.5), 0.25, fill=False, edgecolor=COLORS['warning'],
                         linewidth=2)
            ax.add_patch(circ)
        ...
        # 根据元件类型选择颜色和形状
        if self.element_type == 'polarizer':
            element_color = COLORS['secondary']
            ...
            arrow = FancyArrowPatch((element_x - dx, 2.5 - dy),
                                   (element_x + dx, 2.5 + dy),
                                   arrowstyle='->', mutation_scale=15,
                                   linewidth=SIZES['line_thick'], color=element_color)
            ax.add_patch(arrow)
            ...
            ax.plot([element_x - dx, element_x + dx],
                   [2.5 - dy, 2.5 + dy], color=COLORS['warning'],
                   linewidth=SIZES['line_thick'])
            ...
            ax.plot([element_x - dx_s, element_x + dx_s],
                   [2.5 - dy_s, 2.5 + dy_s], '--', color=COLORS['primary'], linewidth=2)

        elif self.element_type == 'rotator':
            element_color = COLORS['success']
            ...
            element_color = COLORS['text_muted']
        ...
        ax.text(element_x, 1.2, element_name, ha='center', va='top',
               color=element_color, fontsize=FONTS['label']['size'], fontweight='bold')
        ax.text(element_x, 0.8, f'{self.element_angle}°', ha='center', va='top',
               color=element_color, fontsize=FONTS['annotation']['size'])

        # 输出光束
        output_intensity = output_jones.intensity()
        if output_intensity > 0.01:
            output_color = COLORS['success']
            ax.arrow(5.5, 2.5, 2.0, 0, head_width=0.15, head_length=0.2,
                    fc=output_color, ec=output_color,
                    alpha=min(output_intensity, 1), linewidth=SIZES['line_thick'])
        else:
            ax.plot([5.5, 7.5], [2.5, 2.5], ':', color=COLORS['text_muted'],
                   linewidth=2, alpha=0.5)
            ax.text(6.5, 3, 'Blocked', ha='center', color=COLORS['text_muted'],
                   fontsize=FONTS['annotation']['size'], style='italic')

        # 探测器/屏幕
        screen = Rectangle((8.3, 1.7), 0.4, 1.6, facecolor=COLORS['surface_light'],
        ...
        ax.text(2.5, 3.5, f'I_in = {input_intensity:.2f}', ha='center',
               color=COLORS['warning'], fontsize=FONTS['annotation']['size'])
        if output_intensity > 0.01:
            ax.text(7, 3.5, f'I_out = {output_intensity:.2f}', ha='center',
                   color=COLORS['success'], fontsize=FONTS['annotation']['size'])
    ...
    def draw_poincare(self, input_jones, output_jones):
        ...
        ax.scatter([s1_in], [s2_in], [s3_in], color=COLORS['warning'],
        ...
        if output_jones.intensity() > 0.01:
            ...
            ax.scatter([s1_out], [s2_out], [s3_out], color=COLORS['success'],
                      s=100, marker='s', label='Output', zorder=10)

            # 连接线
            ax.plot([s1_in, s1_out], [s2_in, s2_out], [s3_in, s3_out],
                   '--', color=COLORS['text_primary'], linewidth=1, alpha=0.5)

        # 设置
        ax.set_xlim(-1.5, 1.5)
        ax.set_ylim(-1.5, 1.5)
        ax.set_zlim(-1.5, 1.5)
        ax.set_title('Poincaré Sphere', color=COLORS['text_primary'],
                    fontsize=FONTS['subtitle']['size'])
        ax.legend(loc='upper left', fontsize=FONTS['annotation']['size'],
                 facecolor=COLORS['surface'],
                 edgecolor=COLORS['axis'], labelcolor=COLORS['text_primary'])
    ...
    def draw_info_panel(self, input_jones, jones_matrix, output_jones, element_name):
        ...
        ax.text(0.1, 1.8, '【Input Polarization】', fontsize=FONTS['label']['size'],
               fontweight='bold', color=COLORS['warning'])

        input_text = f"Type: {input_type.capitalize()}"
        if input_type == 'linear':
            input_text += f" ({input_params['angle']:.1f}°)"
        ax.text(0.1, 1.4, input_text, fontsize=FONTS['annotation']['size'],
               color=COLORS['text_primary'])

        ax.text(0.1, 1.0, f"Jones: [{input_jones.Ex:.3f}, {input_jones.Ey:.3f}]ᵀ",
               fontsize=FONTS['annotation']['size'], color=COLORS['text_secondary'],
               family='monospace')
        ...
        # Jones矩阵信息
        ax.text(3.5, 1.8, '【Jones Matrix】', fontsize=FONTS['label']['size'],
               fontweight='bold', color=COLORS['primary'])

        ax.text(3.5, 1.4, f"Element: {element_name}", fontsize=FONTS['annotation']['size'],
               color=COLORS['text_primary'])
        ax.text(3.5, 1.0, f"Angle: {self.element_angle}°",
               fontsize=FONTS['annotation']['size'], color=COLORS['text_primary'])

        # 矩阵元素
        m_str = f"J = [[{jones_matrix[0,0]:.2f}, {jones_matrix[0,1]:.2f}], "
        m_str += f"[{jones_matrix[1,0]:.2f}, {jones_matrix[1,1]:.2f}]]"
        ax.text(3.5, 0.6, m_str, fontsize=FONTS['annotation']['size']-1,
        ...
        ax.text(7, 1.8, '【Output Polarization】', fontsize=FONTS['label']['size'],
               fontweight='bold',
               color=COLORS['success'] if output_intensity > 0.01 else COLORS['text_muted'])
        ...
        if output_intensity > 0.01:
            ...
            ax.text(7, 1.4, output_text, fontsize=FONTS['annotation']['size'],
                   color=COLORS['text_primary'])

            ax.text(7, 1.0, f"Intensity: {output_intensity:.3f} ({output_intensity*100:.1f}%)",
                   fontsize=FONTS['annotation']['size'], color=COLORS['text_primary'])
        else:
            ax.text(7, 1.4, "Blocked (I ≈ 0)", fontsize=FONTS['annotation']['size'],
                   color=COLORS['text_muted'], style='italic')

        # 公式提示
        formula = "E_out = J · E_in"
        ax.text(5, 0.1, formula, fontsize=FONTS['subtitle']['size'], color=COLORS['primary'],
...
if __name__ == '__main__':
    print("=" * 70)
    print("Jones矩阵演示 - 优化版 (Jones Matrix Demonstration - OPTIMIZED)")
    print("=" * 70)
    print()
    print("物理原理:")
